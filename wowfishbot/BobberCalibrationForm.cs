...
public sealed partial class BobberCalibrationForm : Form
{
    ...
    private const int SelectionSearchRadius = 2;
    ...
    private void ImageBox_MouseClick(object? sender, MouseEventArgs e)
    {
        ...
        var clickedPoint = GetSourcePoint(e.Location);
        if (clickedPoint is null)
        {
            return;
        }

        if (clickedPoint == cursorLocalPoint)
        {
    ...
    private Point? GetSourcePoint(Point imagePoint)
    {
        if (displayBitmap is null || imagePoint.X < 0 || imagePoint.Y < 0 || imagePoint.X >= imageBox.ClientSize.Width || imagePoint.Y >= imageBox.ClientSize.Height)
        {
            return null;
        }

        var sourceX = (int)((long)imagePoint.X * sourceBitmap.Width / imageBox.ClientSize.Width);
        var sourceY = (int)((long)imagePoint.Y * sourceBitmap.Height / imageBox.ClientSize.Height);
        return new Point(
            Math.Clamp(sourceX, 0, sourceBitmap.Width - 1),
            Math.Clamp(sourceY, 0, sourceBitmap.Height - 1));
    }

    private bool TryFindSelectablePixel(Point clickedPoint, bool requiresRed, bool requiresBlue, out Point sourcePoint, out Color color)
    {
        sourcePoint = Point.Empty;
        color = Color.Empty;
        var bestScore = int.MinValue;

        for (var offsetY = -SelectionSearchRadius; offsetY <= SelectionSearchRadius; offsetY++)
        {
            for (var offsetX = -SelectionSearchRadius; offsetX <= SelectionSearchRadius; offsetX++)
            {
                var candidate = new Point(clickedPoint.X + offsetX, clickedPoint.Y + offsetY);
                if (candidate.X < 0 || candidate.Y < 0 || candidate.X >= sourceBitmap.Width || candidate.Y >= sourceBitmap.Height || candidate == cursorLocalPoint)
                {
                    continue;
                }

                var candidateColor = sourceBitmap.GetPixel(candidate.X, candidate.Y);
                if ((requiresRed && !LooksRed(candidateColor)) || (requiresBlue && !LooksBlue(candidateColor)))
                {
                    continue;
                }
    ...
    private static int GetColorStrength(Color color, bool red) => red
        ? color.R - Math.Max(color.G, color.B)
        : color.B - color.R;

    private static bool LooksRed(Color color) => color.R >= 32 && color.R >= color.G + 4 && color.R >= color.B + 4;

    private static bool LooksBlue(Color color) => color.B >= 32 && color.B >= color.R + 4 && color.B + 16 >= color.G;
